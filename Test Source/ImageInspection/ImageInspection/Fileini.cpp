...
/*static*/ void CIni::AddModulPath(CString& strFileName,BOOL bModulPath /*= TRUE*/)
{
   ...
   _splitpath_s( strFileName, drive,_MAX_DRIVE ,dir,_MAX_DIR, fname,_MAX_FNAME, ext,_MAX_EXT );
   ...
   if( ! drive[0]  )
   {
	  ...
	  _splitpath_s( strModule, drive,_MAX_DRIVE ,dir,_MAX_DIR, fname,_MAX_FNAME, ext,_MAX_EXT );
...
/*static*/ CString CIni::GetDefaultIniFile(BOOL bModulPath /*= TRUE*/)
{
   ...
   _splitpath_s( strTemp, drive,_MAX_DRIVE ,dir,_MAX_DIR, fname,_MAX_FNAME, ext,_MAX_EXT );
...
CPoint CIni::GetPoint(CString strEntry,	CPoint ptDefault, LPCSTR strSection)
{
	...
	sscanf_s(strPoint,"(%d,%d)", &ptReturn.x, &ptReturn.y);
...
CRect CIni::GetRect(CString strEntry, CRect rectDefault, LPCSTR strSection)
{
	...
	sscanf_s(strRect,"(%d,%d,%d,%d)", &rectReturn.top,&rectReturn.left,&rectReturn.bottom,&rectReturn.right);
...
COLORREF CIni::GetColRef(CString strEntry, COLORREF crDefault, LPCSTR strSection)
{
	...
	sscanf_s(strColRef,"RGB(%d,%d,%d)", temp, temp+1, temp+2);
...
void		CIni::SerGet(	BOOL bGet,unsigned int		& n,	CString strEntry,	LPCSTR strSection/*= NULL*/,	int nDefault/* = 0*/)
{
	int nTemp = (int)n;
...
void		CIni::SerGet(	BOOL bGet,unsigned short		& n,	CString strEntry,	LPCSTR strSection/*= NULL*/,	int nDefault/* = 0*/)
{
   int nTemp = (int)n;
   SerGetInt(bGet,nTemp,strEntry,strSection,nDefault);
   n = nTemp;
...
void CIni::SerGet(BOOL bGet,int * ar, int nCount1, int nCount2, CString strEntry, LPCSTR strSection/*=NULL*/, int Default/* = 0*/)
{
	int *base;
	CString strBuffer;
	base = ar;
	for( int i=0 ; i<nCount1 ; i++, base+=nCount2)
	{
	   for( int j=0 ; j<nCount2 ; j++)
	   {
		  strBuffer.Format("[%d][%d]",i,j);
		  strBuffer = strEntry + strBuffer;

		  SerGet(bGet,base[j],strBuffer,strSection,Default);
	   }
	}
}
