...
public:
    ...
    void RemoveLayer (FCObjLayer* pLayer)
    {
        ...
        int     nIndex = pt - m_LayerList.begin() ;
                         ...
                         FCObjImage& imgRegion) const
    {
        ...
        // must in canvas
        {
            RECT     rcCanvas = {0,0,0,0} ;
            rcCanvas.right = GetCanvasDimension().cx ;
            rcCanvas.bottom = GetCanvasDimension().cy ;
        ...
        for (size_t i=0 ; i < layerList.size() ; i++)
        {
            ...
            OffsetRect (&rcOnImg, -rcRegion.left, -rcRegion.top) ;
            rcOnImg.left   = (int)(fScale * rcOnImg.left) ;
            rcOnImg.top    = (int)(fScale * rcOnImg.top) ;
            rcOnImg.right  = (int)(fScale * rcOnImg.right) ;
            rcOnImg.bottom = (int)(fScale * rcOnImg.bottom) ;
    ...
    bool Save_oXo (const char* szFileName) const
    {
        ...
        *(DWORD*)pCurr = 0 ;           ; pCurr += 4 ;
        ...
        // write file
        return FCOXOHelper::SaveBufferToFile (szFileName, pStart.get(), pCurr - pStart.get()) ;
    }
    //@}
