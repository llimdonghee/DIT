...
AFX_STATIC_DATA HPEN _afxDottedPen = 0;
...
void AFX_CDECL AfxTrackerTerm()
{
	...
	AfxDeleteObject((HGDIOBJ*)&_afxDottedPen);
}
...
/////////////////////////////////////////////////////////////////////////////
// CRcTracker intitialization

CRcTracker::CRcTracker()
{
	Construct();
}

CRcTracker::CRcTracker(LPCRECT lpSrcRect, UINT nStyle)
{
...
void CRcTracker::Construct()
{
	m_crLine = RGB(0, 255, 255);
	m_crHandle = RGB(0, 0, 255);
	...
	if (!bInitialized)
	{
		...
		{
			// create black dotted pen
			_afxDottedPen = CreatePen(PS_DOT, 0, m_crLine);
			if (_afxDottedPen == NULL)
			{
				AfxUnlockGlobals(CRIT_RECTTRACKER);
				AfxThrowResourceException();
...
/////////////////////////////////////////////////////////////////////////////
// CRcTracker operations

void CRcTracker::Draw(CDC* pDC) const
{
	...
	if ((m_nStyle & (resizeInside|resizeOutside)) != 0)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			if (mask & (1<<i))
			{
				...
				pDC->FillSolidRect(rect, m_crHandle);
			}
		}
	}
...
BOOL CRcTracker::SetCursor(CWnd* pWnd, UINT nHitTest) const
{
...
int CRcTracker::HitTest(CPoint point) const
{
...
int CRcTracker::NormalizeHit(int nHandle) const
{
...
BOOL CRcTracker::Track(CWnd* pWnd, CPoint point, BOOL bAllowInvert,
						 CWnd* pWndClipTo)
{
...
BOOL CRcTracker::TrackRubberBand(CWnd* pWnd, CPoint point, BOOL bAllowInvert)
{
...
void CRcTracker::DrawTrackerRect(
...
void CRcTracker::AdjustRect(int nHandle, LPRECT)
{
...
void CRcTracker::GetTrueRect(LPRECT lpTrueRect) const
{
...
void CRcTracker::OnChangedRect(const CRect& /*rectOld*/)
{
	// no default implementation, useful for derived classes
}

/////////////////////////////////////////////////////////////////////////////
// CRcTracker implementation helpers

void CRcTracker::GetHandleRect(int nHandle, CRect* pHandleRect) const
{
	ASSERT(nHandle < 8);
...
int CRcTracker::GetHandleSize(LPCRECT lpRect) const
{
	if (lpRect == NULL)
		lpRect = &m_rect;
...
int CRcTracker::HitTestHandles(CPoint point) const
{
	CRect rect;
	UINT mask = GetHandleMask();
...
BOOL CRcTracker::TrackHandle(int nHandle, CWnd* pWnd, CPoint point,
							   CWnd* pWndClipTo)
{
	ASSERT(nHandle >= 0);
...
void CRcTracker::GetModifyPointers(
...
UINT CRcTracker::GetHandleMask() const
{
