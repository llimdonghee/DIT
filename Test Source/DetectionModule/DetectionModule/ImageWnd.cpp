...
{
	if(m_bPaintDefect == TRUE && m_pDetectionAlgorithm != NULL)
	{		
		CPoint po;
		for(int iDefect = 0; iDefect < m_pDetectionAlgorithm->m_Merge.iDefectCnt; iDefect++)
		{
			if(m_pDetectionAlgorithm->m_Merge.DefectList[iDefect].bDelete == TRUE)
				continue;

			for(int iPixel = 0; iPixel < m_pDetectionAlgorithm->m_Merge.DefectList[iDefect].iPixelCnt; iPixel++)
			{
				po.x = m_pDetectionAlgorithm->m_Merge.DefectList[iDefect].PixelList[iPixel].iX;
				po.y = m_pDetectionAlgorithm->m_Merge.DefectList[iDefect].PixelList[iPixel].iY;				
...
{
	if(m_bCalcRealFOV == TRUE && m_pDetectionAlgorithm != NULL)
	{
		m_bCalcRealFOV = FALSE;
		...
		double dblPixelSize = m_pDetectionAlgorithm->m_Param.m_iHDist*m_pDetectionAlgorithm->m_Param.m_fFOV;
		double dblDefaultFOV = m_pDetectionAlgorithm->m_Param.m_fFOV;
...
void CImageViewWnd::ModifyIndicator_ImageSize(CSize szImage, CSize szView)
{
	...
	m_pDlgDetectionModule->SetDlgItemText(IDC_STATIC_IMAGE_SIZE, str);
	m_pDlgDetectionModule->m_iImageSizeX = szImage.cx;
	m_pDlgDetectionModule->m_iImageSizeY = szImage.cy;
}
...
void CImageViewWnd::ModifyIndicator_Zoom(float fZoom)
{
	...
	m_CSZoomRate = str;
	m_pDlgDetectionModule->SetDlgItemText(IDC_STATIC_IMAGE_ZOOM, str);
	m_pDlgDetectionModule->ZoomRateValue(fZoom);
}
...
{
	CString str;
	str.Format(" [Image X : %d, Y : %d] [Threshold : %d]", 
		ptPos.x, ptPos.y, btThreshold);

	m_pDlgDetectionModule->SetDlgItemText(IDC_STATIC_IMAGE_POSITION, str);
}
...
{
	CString str = "";
	str.Format(" Gray Level : [Vertical : %03d-%03d-%03d-[%03d]-%03d-%03d-%03d (Diff : %03d-%03d-%03d} / [Horizontal : %03d-%03d-%03d-[%03d]-%03d-%03d-%03d == Hori Gray Diff : %03d-%03d-%03d]",
	...
	m_pDlgDetectionModule->SetDlgItemText(IDC_STATIC_IMAGE_INFO, str);
}
...
{	
	...
	{		
		...
		else if (abs(m_rtCalcRect.Width()) >= CALC_MIN && abs(m_rtCalcRect.Height()) >= CALC_MIN)
		{
			...
			{
				m_pDlgDetectionModule->SetRectCoordinate(rcCalcO);
			}
			...
			// FOV 값 기재
			//str.Format("(dx:%d, dy:%d)  Measure FOV = %f um", rcCalcO.Width(), rcCalcO.Height(), rcCalcO.Width() / m_pDlgDetectionModule->m_dPixelsize);
...
void CImageViewWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_DrawImage.IsValidImage())
	{
		CRect rtROI = m_pDetectionAlgorithm->m_Param.m_rtROI;
		int iVDist = m_pDetectionAlgorithm->m_Param.m_iVDist, iHDist = m_pDetectionAlgorithm->m_Param.m_iHDist;
		...
		if (po.x > 0 && po.x < (int)m_DrawImage.Width() - 1 && po.y > 0 && po.y < (int)m_DrawImage.Height() - 1)
		{
			...
			else if(po.x + iHDist + 1 > rtROI.right - 1)
			{
				if(po.x - 2*iHDist >= 0)
				{
					dwLeft = m_OriginImage.GetPixelData(po.x - iHDist, po.y);
			...
			else if (po.x + iMHDist + 1 > rtROI.right - 1)
			{
				if (po.x - 2 * iMHDist >= 0)
				{
					dwMultiLeft = m_OriginImage.GetPixelData(po.x - iMHDist, po.y);
			...
			else if (po.x + iTHDist + 1 > rtROI.right - 1)
			{
				if (po.x - 2 * iTHDist >= 0)
				{
					dwThirdLeft = m_OriginImage.GetPixelData(po.x - iTHDist, po.y);
			...
			dwThreshold = (DWORD)((double)((m_pDetectionAlgorithm->m_Param.m_iThresholdHigh - m_pDetectionAlgorithm->m_Param.m_iThresholdLow)) / 256.0 * (BYTE)dwImage + m_pDetectionAlgorithm->m_Param.m_iThresholdLow);
